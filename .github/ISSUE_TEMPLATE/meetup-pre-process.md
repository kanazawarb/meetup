...
* レポート公開後にチャネルのログを自動でアーカイブするため、アーカイブ用の bot をチャネルに招待してください
    * チャネルで `/invite @(bot 名)` を実行します
    * bot が参加していないとアーカイブ作成のワークフローが失敗します
    * アーカイブに残したくない投稿には `:shushing_face:` もしくは `:speak_no_evil:` をリアクションで付けてください
        * 本文にこれらの絵文字を含む投稿も同様に除外されます
        * 除外はレポートのマージ時点の状態で判定されるため、それまでに付けておけば間に合います
